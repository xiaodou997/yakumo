...
pub async fn activate_license<R: Runtime>(
    ...
    let client = yakumo_api_client(ApiClientKind::App, &app_version)?;
...
pub async fn check_license<R: Runtime>(window: &WebviewWindow<R>) -> Result<LicenseCheckStatus> {
    ...
    match (has_activation_id, trial_period_active) {
        ...
        (true, _) => {
            ...
            let client = yakumo_api_client(ApiClientKind::App, &payload.app_version)?;
...
fn build_url(path: &str) -> String {
    if is_dev() {
        ...
        format!("https://license.yakumo.local{path}")
    }
}
