...
use yakumo_common::serde::{get_bool, get_bool_map, get_str, get_str_map};
...
mod tests {
    ...
    use serde_json::json;
    use std::collections::BTreeMap;
    use yakumo_models::models::{HttpRequest, HttpUrlParameter};
    ...
    fn test_build_url_with_existing_query_and_fragment() {
        let r = HttpRequest {
            url: "https://example.com?foo=bar#some-hash".to_string(),
            url_parameters: vec![HttpUrlParameter {
                enabled: true,
                name: "baz".to_string(),
                value: "qux".to_string(),
        ...
        let result = build_url(&r);
        assert_eq!(result, "https://example.com?foo=bar&baz=qux#some-hash");
    }
