...
export async function listYakuWorkspaces(limit = 500) {
  return invokeCmd<YakuPageResponse<YakuWorkspacePageItem>>(
...
export async function getYakuWorkspace(workspaceId: string) {
  return invokeCmd<YakuWorkspace | null>("cmd_yaku_workspace_get", {
    workspaceId,
  });
}

export async function createYakuWorkspace(name: string, description = "") {
  return invokeCmd<YakuWorkspace>("cmd_yaku_workspace_create", {
    name,
    description,
  });
}
...
export async function deleteYakuWorkspace(workspaceId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_workspace_delete", {
    workspaceId,
  });
}

export async function listYakuEnvironments(workspaceId: string) {
  return invokeCmd<YakuEnvironment[]>("cmd_yaku_environment_list", {
    workspaceId,
  });
}

export async function getYakuEnvironment(environmentId: string) {
  return invokeCmd<YakuEnvironment | null>("cmd_yaku_environment_get", {
    environmentId,
  });
}
...
export async function createYakuEnvironment(
  ...
  return invokeCmd<YakuEnvironment>("cmd_yaku_environment_create", {
    workspaceId,
    name,
    variables,
  });
...
export async function deleteYakuEnvironment(environmentId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_environment_delete", {
    environmentId,
  });
}

export async function listYakuCookieJars(workspaceId: string) {
  return invokeCmd<YakuCookieJar[]>("cmd_yaku_cookie_jar_list", {
    workspaceId,
  });
}

export async function getYakuSecretAudit(workspaceId: string) {
  return invokeCmd<YakuSecretAudit>("cmd_yaku_secret_audit", { workspaceId });
}

export async function deleteYakuSecret(secretId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_secret_delete", { secretId });
}

export async function deleteYakuOrphanSecrets(workspaceId: string) {
  return invokeCmd<YakuSecretCleanupResponse>("cmd_yaku_secret_orphans_delete", {
    workspaceId,
  });
}

export async function createYakuCookieJar(workspaceId: string, name: string) {
  return invokeCmd<YakuCookieJar>("cmd_yaku_cookie_jar_create", {
    workspaceId,
    name,
  });
}
...
export async function deleteYakuCookie(cookieId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_cookie_delete", { cookieId });
}
...
export async function listYakuGrpcServices(input: {
  url: string;
  metadata?: Record<string, string>;
  protoFiles?: string[];
  protoImportRoots?: string[];
  useReflection?: boolean;
}) {
  return invokeCmd<YakuGrpcServiceDefinition[]>("cmd_yaku_grpc_services", {
    url: input.url,
    metadata: input.metadata ?? {},
    protoFiles: input.protoFiles ?? [],
    protoImportRoots: input.protoImportRoots ?? [],
    useReflection: input.useReflection ?? true,
  });
}

export async function listYakuRequests(workspaceId: string, limit = 500) {
  return invokeCmd<YakuPageResponse<YakuRequestNodePageItem>>(
...
export async function updateYakuFolder(
  folderId: string,
  input: { name?: string },
) {
  return invokeCmd<YakuRequestNode>("cmd_yaku_folder_update", {
    folderId,
    name: input.name,
  });
}
...
export async function updateYakuRequest(
  ...
  return invokeCmd<YakuRequest>("cmd_yaku_request_update", {
    requestId,
    ...input,
  });
}
...
export async function deleteYakuRequestNode(nodeId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_request_node_delete", {
    nodeId,
  });
}

export async function listYakuRunsForRequest(requestId: string, limit = 100) {
  return invokeCmd<YakuPageResponse<YakuRunPageItem>>("cmd_yaku_run_list", {
    requestId,
    limit,
  });
}

export async function listYakuRunsForWorkspace(
  workspaceId: string,
  limit = 100,
) {
  return invokeCmd<YakuPageResponse<YakuRunPageItem>>("cmd_yaku_run_list", {
...
export async function startYakuRun(
  requestId: string,
...
export async function getYakuRunRetention(workspaceId: string) {
  return invokeCmd<number | null>("cmd_yaku_run_retention_get", {
    workspaceId,
  });
}

export async function setYakuRunRetention(
  workspaceId: string,
  keepLast: number,
) {
  return invokeCmd<YakuSetting>("cmd_yaku_run_retention_set", {
    workspaceId,
    keepLast,
  });
}
...
export async function clearYakuRunRetention(workspaceId: string) {
  return invokeCmd<YakuDeleteResponse>("cmd_yaku_run_retention_clear", {
    workspaceId,
  });
}
...
export async function exportYakuWorkspaceBackup(
  workspaceId: string,
  exportPath: string,
) {
  return invokeCmd<YakuBackupManifest>("cmd_yaku_backup_export", {
    workspaceId,
    exportPath,
  });
}

export async function importYakuWorkspaceBackup(
  filePath: string,
...
export async function sendYakuRequest(
  requestId: string,
  environmentId?: string | null,
) {
