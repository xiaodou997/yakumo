...
export class YakumoColor {
  ...
  static transparent(): YakumoColor {
    return new YakumoColor("rgb(0,0,0)", "light").translucify(1);
  }

  static white(): YakumoColor {
    return new YakumoColor("rgb(0,0,0)", "light").lower(1);
  }

  static black(): YakumoColor {
    return new YakumoColor("rgb(0,0,0)", "light").lift(1);
  }

  set(cssColor: string): YakumoColor {
  ...
  clone(): YakumoColor {
    return new YakumoColor(this.css(), this.appearance);
  }

  lower(mod: number): YakumoColor {
    return this.appearance === "dark" ? this._darken(mod) : this._lighten(mod);
  }

  lift(mod: number): YakumoColor {
    return this.appearance === "dark" ? this._lighten(mod) : this._darken(mod);
  }

  minLightness(n: number): YakumoColor {
    const c = this.clone();
    if (c.lightness < n) {
      c.lightness = n;
    }
  ...
  translucify(mod: number): YakumoColor {
    const c = this.clone();
    c.alpha = c.alpha - c.alpha * mod;
    return c;
  }

  opacify(mod: number): YakumoColor {
    const c = this.clone();
    c.alpha = this.alpha + (100 - this.alpha) * mod;
    return c;
  }

  desaturate(mod: number): YakumoColor {
    const c = this.clone();
    c.saturation = c.saturation - c.saturation * mod;
    return c;
  }

  saturate(mod: number): YakumoColor {
    const c = this.clone();
    c.saturation = this.saturation + (100 - this.saturation) * mod;
    return c;
  }

  lighterThan(c: YakumoColor): boolean {
  ...
  private _lighten(mod: number): YakumoColor {
    const c = this.clone();
    c.lightness = this.lightness + (100 - this.lightness) * mod;
    return c;
  }

  private _darken(mod: number): YakumoColor {
    const c = this.clone();
